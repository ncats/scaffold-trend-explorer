(function () {
    'use strict';
    var ginasApp = angular.module('ginas', ['ngAria', 'ngMessages', 
    	'ngResource', 'ui.bootstrap', 
    	'ui.bootstrap.showErrors','LocalStorageModule',
    	'ngTagsInput','jsonFormatter', 
    	'ginasForms','ginasFormElements', 
    	'ginasAdmin','ginasDownloads',
        'ginasScheduled', 'diff-match-patch',
        'angularSpinners', 'filterListener', 
        'validatorListener', 'ginasFilter'
    ...
    ginasApp.controller("GinasController", function ($rootScope, $scope, $resource, $location, $compile, $uibModal, $http, $window, $anchorScroll, $timeout, polymerUtils,
        ...
        console.log("Submitting");
            ...
            var old=$location.search().q;
            if(old){
            	$scope.q="(" + old + ") AND (" + $scope.q + ")";
            }
            //$location.path(".." + action);
            $location.search("q",$scope.q);
            $location.hash("");
        ...
        $scope.sortValues = [
        	{
                "value": "$Approval Year",
                "display": "Approval Year, Recent"
            },
            {
                "value": "^Approval Year",
                "display": "Approval Year, Oldest"
            },
        	{  
               "value":  "default",
               "display": "Relevance"
...
//this is already a function the substance object has, not really needed.
        ...
        $scope.getRange = function (start, end) {
            return _.reverse(_.range(start, (end - 0 + 1)));
        };
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.relink = function(){
                    var url = baseurl + 'img/' + scope.id + '.svg?size={{size||200}}';
                    ...
                    if (attrs.smiles) {
                        var smiles = attrs.smiles
                                            ...
                                            .replace(/[+]/g,'%2B')
                                            .replace(/[|]/g,'%7C');
                        url = baseurl + "render?structure=" + smiles + "&size={{size||200}}&standardize=true";
                    }
                    var template = angular.element('<img width=height={{size||200}} height={{size||200}} ng-src="' + url + '" class="tooltip-img" ng-cloak>');
  ...
  var dir = {
    restrict: 'E',
    scope:{},
    xemplate: '<a class="info-pop" uib-popover-template="\'htmlvar\'"><i class="fa fa-info-circle"></i></a>',
    compile: function(element, attrs, linker) {
      ...
      var elm=angular.element(template + dir.xemplate.replace("htmlvar",rnd));
      
      //element.html(dir.xemplate); // set template html manually
      
      return function(scope, element, attributes) {
        scope.test123="message";
...
ginasApp.filter('refContains', function() {
  return function(input, optional1, optional2) {
  	if(!optional1)return input;
    var all = _.chain(optional1)
               .map(function(o){return o.term;})
               .value();
               
	var tot= 
		_.chain(input)
		 .filter(function(m){
			 return _.indexOf(all,m.uuid)>=0;
		  })
		 .value();
    ...
    ginasApp.initializeReferenceView = function(scope){
    	if(scope.show){
    		_.forEach(scope.show, function(s){
    			scope["_" + s]=true;
    		});
    	}else{
    		scope._index=true;
    ...
    ginasApp.directive('reftable', function () {
        return {
            ...
            link: function (scope, element, attrs, referencesCtrl) {
            	...
            	ginasApp.initializeReferenceView(scope);
            	
            	if(scope.show && scope.show.length==1){
            		scope._tabClass="single-table";
            	}else{
            		scope._tabClass="table kv-table";
            	}
            	
            	
                referencesCtrl.referenceRetriever
			                .getAll(scope.substance)
			                .then(function (response) {
    ...
    ginasApp.directive('refCount', function ($compile) {
        return {
            ...
            link: function (scope, element, attrs, referencesCtrl) {
                ...
                if(!scope.references){
                	scope.references=scope.sreferences;
                }
                ginasApp.initializeReferenceView(scope);

                referencesCtrl.referenceRetriever.getAll(scope.substance)
                                                 .then(function (response) {
								                    _.forEach(scope.references, function (ref) {
								                        scope.objreferences.push(_.find(response, ['uuid', ref.term]));
								                        indices.push(_.indexOf(response, _.find(response, ['uuid', ref.term])) + 1);
								                        // });
								                    });
								                    indices = _.sortBy(indices);
    ...
    ginasApp.directive("treeView", function ($compile) {
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                var tog='<span class="tree-toggle" ng-click="showTree=!showTree"><i ng-hide="showTree" class="fa fa-plus-square" aria-hidden="true"></i><i ng-show="showTree" class="fa fa-minus-square" aria-hidden="true"></i></span>';
                
                var template = '<span ng-show="showTree">';
                ...
                _.forEach(scope.codes, function (c, key) {
                    ...
                    var mtree = '<ul class="tree-list"><li><a class="label label-primary loader" href = "' + link + '" uib-tooltip="Search for ' + c[0] + '" target ="_self">' + c[0] + '</a>';
                    
                    
                    template += mtree;
                    if (c[1]) {
                        template += '<span ng-show="showDetails">' + '[' + c[1] + '</span>';
                    }
                    
                    if(i===scope.codes.length-1){
                    	single=mtree + "</ul>";
                    }
                });
                ...
                template = "<div ng-show='text' class='tree-view'><table><tbody><tr><td>" +  tog + "</td><td><span title='Show Full Tree' ng-hide='showTree'>" + single + "</span>" + template + "</td></tr></tbody></table> </div>";
